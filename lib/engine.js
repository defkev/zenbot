...
module.exports = function (s, conf) {
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    let price, fee, expected_fee, buy_pct, sell_pct, reorder_pct, trade_balance, tradeable_balance, trades
        ...
        if (signal === 'buy') {
          ...
          if (!size) {
            if (so.mode === 'live' || so.mode === 'paper') {
              ...
              size = n(s.balance.currency).multiply(buy_pct).divide(100).divide(price).format('0.00000000')
            }
          }
